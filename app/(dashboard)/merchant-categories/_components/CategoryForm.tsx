...
export function CategoryForm({
  ...
  // Load selected category into state when clicked
  useEffect(() => {
    if (selectedCategory) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const savedCategory: CategoryItem = {
      id: selectedCategory ? selectedCategory.id : `cat-${Date.now()}`,
      name: catName,
      code: catCode.toUpperCase(),
      description: catDescription || "New sector...",
      linkedMerchants: selectedCategory ? selectedCategory.linkedMerchants : 0,
      taxRule: taxPolicy.includes("SC 10%")
        ? "VAT 15% + SC 10%"
  ...
  return (
    <div id="category-form" className="rounded-2xl border border-border bg-card p-5 shadow-2xs space-y-4">
      <div className="flex items-start justify-between border-b border-border/80 pb-3">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">Configure sector classification</p>
        </div>
      <RotateCcw
  ...
  <title>Reset form</title>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <CustomInput
          ...
          value={catCode}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCatCode(e.target.value)}
          maxLength={3}
          required
        />
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            className="w-full rounded-xl border border-transparent bg-indigo-50/50 dark:bg-slate-900/60 p-3 text-xs font-semibold text-foreground focus:border-indigo-500 focus:bg-background focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder:text-muted-foreground/60 resize-none"
          />
        </div>
        ...
        <CustomInput
          ...
          as="select"
          value={taxPolicy}
          onChange={(e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => setTaxPolicy(e.target.value)}
        >
          <option value="VAT 15% (Standard Output)">VAT 15% (Standard Output)</option>
        ...
        <div className="flex items-center justify-between rounded-xl bg-indigo-50/40 dark:bg-slate-900/40 p-3 border border-border/60">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Enabled for new merchants</p>
          </div>
          <button
            type="button"
            ...
            className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-hidden ${
              isCategoryEnabled ? "bg-indigo-600" : "bg-slate-300 dark:bg-slate-700"
            }`}
          >
            <span
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          >
            ...
            <span>{selectedCategory ? "Update Category" : "Save Category"}</span>
          </Button>
        </div>
      </form>
    </div>
